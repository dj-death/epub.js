import { qs, sprint, locationOf, defer, size } from './utils/core'
...
class Locations {
  ...
  async generateAsync (chars) {
    ...
    return this.q.enqueue(this.processAsync.bind(this), sections)
  }

  async processAsync (sectionOrSections) {
    ...
    return new Promise((resolve, reject) => {
      Promise.all(sections.map((section, index) => {
        ...
        const allCompleted = new defer()

        // Process sequentially
        console.debug('start of section 1')

        for (let i = 0, len = loadedContents.length; i < len; i++) {
          ...
          this.processingTimeout = setTimeout(() => {
            console.debug('section ' + i + ' parsed')
  ...
  process (section) {
    ...
    return section.load(this.request)
      .then(function (contents) {
        ...
        return new Promise((resolve, reject) => {
          setTimeout(() => {
            const locations = this.parse(contents, section.cfiBase, null, section.index)
            resolve(locations)
          })
        }).then((locations) => {
          this._locations = this._locations.concat(locations)
  ...
  parse (contents, cfiBase, chars, sectionIndex) {
    ...
    const nodesCount = Object(size)(body)
    let i = 0
    const self = this
    ...
    const parser = function (node) {
      ...
      const progressPercent = nodesCount ? Math.round(i * 100 / nodesCount) : 100
      if (progressPercent - lastProgress >= 25) {
        lastProgress = progressPercent

        self.emit(EVENTS.LOCATIONS.PARSING, progressPercent, sectionIndex, self.sectionsCount)
      }
    }
