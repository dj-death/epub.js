...
class Rendition {
  ...
  _display (target) {
    ...
    // console.debug(section, target)

    const self = this
    section.on(EVENTS.SECTION.CONTENT_LOADED,
      self.emit.bind(self, EVENTS.RENDITION.SECTION_CONTENT_LOADED, section.index))

    section.on(EVENTS.SECTION.CONTENT_PARSED,
      self.emit.bind(self, EVENTS.RENDITION.SECTION_CONTENT_PARSED, section.index))

    section.on(EVENTS.SECTION.RENDERING_PROGRESS, (progress) => {
      self.emit(EVENTS.RENDITION.SECTION_RENDERING, progress, section.index)
    })

    section.on(EVENTS.SECTION.PARSING_PROGRESS,
      self.emit.bind(self, EVENTS.RENDITION.SECTION_PARSING, section.index))
  ...
  adjustImages (contents) {
    if (this._layout.name === 'pre-paginated') {
      ...
      return new Promise(function (resolve) {
        that.emit(EVENTS.RENDITION.ADJUSTING_IMAGES, that)
        resolve()
      })
    }
    ...
    return new Promise(function (resolve, reject) {
      // Wait to apply
      setTimeout(function () {
        self.emit(EVENTS.RENDITION.ADJUSTING_IMAGES, self)
        resolve()
      }, 1)
    })
  }
