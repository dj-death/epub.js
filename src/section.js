...
class Section {
  ...
  load (_request) {
    ...
    if (this.contents) {
      ...
      request(this.url)
        ...
        .then(function () {
          loading.resolve(this.contents)
          this.emit(EVENTS.SECTION.CONTENT_LOADED, this)
        }.bind(this))
  ...
  render (_request) {
    ...
    this.output = null // TODO: better way to return this from hooks?
    ...
    this.load(_request)
      ...
      .then(function () {
        ...
        this.emit(EVENTS.SECTION.DISPLAYED, this)
      }.bind(this))
      .catch(function (error) {
  ...
  search (_query, maxSeqEle = 5) {
    ...
    const search = function (nodeList) {
      ...
      if (pos != -1) {
        const startNodeIndex = 0; const endPos = pos + query.length
        let endNodeIndex = 0; let l = 0
        if (pos < nodeList[startNodeIndex].length) {
          ...
          const startNode = nodeList[startNodeIndex]; const endNode = nodeList[endNodeIndex]
          const range = section.document.createRange()
          range.setStart(startNode, pos)
          const beforeEndLengthCount = nodeList.slice(0, endNodeIndex).reduce((acc, current) => { return acc + current.textContent.length }, 0)
...
// -- Enable binding events to Section
