...
class IframeView {
  ...
  render (request, show) {
    ...
    const progressUpdater = function (start, max) {
      self.section.emit(EVENTS.SECTION.RENDERING_PROGRESS, start, self.section.index, self.sectionsCount)
      let progress = start
      const diff = max - start

      progressTimer = setInterval(function () {
        progress += Math.round(diff / 5)
        if (progress < max) {
          this.section.emit(EVENTS.SECTION.RENDERING_PROGRESS, progress, self.section.index, self.sectionsCount)
    ...
    progressUpdater(5, 10)

    // Render Chain
    return this.sectionRender
      .then(function (contents) {
        progressUpdater(10, 40)
        return this.load(contents, function () {
          progressUpdater(40, 60)
        })
      }.bind(this))
      .then(function () {
        ...
        return new Promise((resolve, reject) => {
          // progressUpdater(70, 80)
          // Expand the iframe to the full size of the content
          this.expand(false, function () {
            progressUpdater(80, 90)
          })
      ...
      .then(function () {
        ...
        this.emit(EVENTS.VIEWS.RENDERING_PROGRESS, 100)
      }.bind(this))
  }
  ...
  load (contents, onLoadCallback) {
    ...
    this.iframe.onload = function (event) {
      if (typeof (onLoadCallback) === 'function') onLoadCallback.call(self)
