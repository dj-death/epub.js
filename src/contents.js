...
class Contents {
  ...
  /**
		...
		*/
  scrollWidth () {
    let width = this.documentElement.scrollWidth
  ...
  addStylesheetRules (rules, key) {
    let styleSheet
      ...
      for (let i = 0, rl = rules.length; i < rl; i++) {
        let j = 1; var rule = rules[i]; var selector = rules[i][0]; var propStr = '';
  ...
  /**
	 ...
	 */
  size (width, height) {
    let viewport = { scale: 1.0, scalable: 'no' }
  ...
  fit (width, height, section) {
    let viewport = this.viewport()
      ...
      let marginLeft = width - (viewportWidth * scale)
  ...
  mapPage (cfiBase, layout, start, end, dev) {
    let mapping = new Mapping(layout, dev)
